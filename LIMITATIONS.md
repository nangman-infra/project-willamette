...
| **LUT kernel (bitnet.cpp TL1/TL2 style)** | not implemented; **design RFC committed 2026-05-30** ([`docs/LUT_KERNEL_RFC.md`](docs/LUT_KERNEL_RFC.md)). Plan-then-act: a *scalar LUT* prototype runs first as a measurement (gate: ≥ 1.3× over scalar BitLinear on antix1 or mbp2012), then an SSSE3 `pshufb` variant on hosts that detect it (gate: ≥ 1.5× over scalar LUT on mbp2012). Pentium-M's SSE2 ceiling still excludes `pshufb` but scalar LUT works there. |
