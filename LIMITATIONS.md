...
| **LUT kernel (scalar prototype)** | **step-1 measurement landed 2026-05-30**: scalar LUT clears every gate and **beats the SSE2 i8 production kernel by 5.29× on antix1** (Pentium-M). Loses by 2.5× on mbp2012 (Ivy Bridge SSE2 i8 wins there). RFC step 3 (dispatch integration on SSE2-only hosts) is the next code cycle. SSSE3 `pshufb` LUT (step 4) is no longer on the critical path since mbp2012-class hosts already win without it. See [`docs/LUT_KERNEL_RFC.md`](docs/LUT_KERNEL_RFC.md) § 5 step-1 outcome + `docs/BENCHMARKS.md` 2026-05-30 § "LUT step-1 prototype measurement". |
